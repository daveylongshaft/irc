...
try:
    from csc_data.services.stats_service.stats_service import StatsService
except (ImportError, ModuleNotFoundError):
    StatsService = None  # sqlite3 not available on this build
try:
    from csc_cli.csc_clients.jules.jules import Jules
    from csc_cli.csc_clients.jules.config import JulesConfig
except (ImportError, ModuleNotFoundError):
    Jules = None
    JulesConfig = None
...
def prioritize(filename: str) -> str:
    ...
    if any(kw in fn for kw in ["queue_worker", "queue-worker", "test_runner",
                                ...
                                "infrastructure", "csc_loop", "csc-service",
                                "csc_ctl", "csc-ctl"]):
        return "P1"
    # P3: documentation
    if "docs_" in fn or "docstring" in fn or "document_" in fn:
...
def monitor_julius_plans() -> None:
    ...
    try:
        from csc_cli.csc_clients.julius.julius import Julius
    except ImportError:
...
def run_cycle() -> list:
    ...
    for wo_file, priority, category, entry in candidates:
        ...
        try:
            ...
            from csc_data.services.agent_service import Agent as AgentService
