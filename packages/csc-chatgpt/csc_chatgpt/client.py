...
class ChatGPTClient(AIClient):
    ...
    def __init__(self, config_path=None, input_file=None, output_file=None):
        ...
        model = self._perform.get("ai", "model", "gpt-4o")
        self._api = ChatGPTAPI(api_key, model=model)
    ...
    def _resolve_api_key(self):
        ...
        if key:
            return key
        raise ValueError(
            "[ChatGPTClient] CRITICAL: No API key found. "
            "Set OPENAI_API_KEY env var or store chatgpt_api_key in the data store."
        )
