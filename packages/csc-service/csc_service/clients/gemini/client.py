...
from csc_service.clients.gemini.aliases import Aliases
from csc_service.clients.gemini.macros import Macros
...
class Client(Network):
    ...
    def __init__(self, config_path=None, host=None, port=None):
        ...
        host = host or os.getenv("CSC_SERVER_HOSTNAME", "127.0.0.1")
        try:
        ...
        super().__init__(host=host, port=port, name="client")
        ...
        self._isop_cache = {}
        self._isop_cache_ttl = 30  # seconds
    ...
    def _handle_numeric(self, parsed):
        ...
        if num == "433":
            ...
            self.log(f"[AUTO] Nickname '{old_nick}' in use. Trying '{self.name}'.")
            self.identify()
            return
    ...
    def _handle_privmsg_recv(self, parsed):
        ...
        Returns:
            True if the message was handled (CTCP, ACTION, or nick-prefixed command)
            False if the message should be processed by the AI model
        """
        nick = parsed.prefix.split("!")[0] if parsed.prefix else "?"
        target = parsed.params[0] if parsed.params else "?"
        ...
        # Check for nick-prefixed command: "<own_nick> AI <token> <class> <method> [args]"
        if text.startswith(f"{self.name} AI ") or text.startswith(f"{self.name} <begin file="):
            # Auth check: sender has +o in any shared channel, or is server itself
            sender_authorized = False

            # Check if sender is the server
            if "@csc-server" in prefix_full or "@{SERVER_NAME}" in prefix_full:
                sender_authorized = True

            # Check if sender has +o in any shared channel
            if not sender_authorized:
                for chan, ops in self.channel_ops.items():
                    if nick in ops:
                        sender_authorized = True
                        break
            ...
            if not sender_authorized:
                ...
                cached = self._isop_cache.get(nick)
                if cached and (_time.time() - cached[1]) < self._isop_cache_ttl:
                    sender_authorized = cached[0]
                else:
                    # Query ISOP asynchronously — for now check cache only
                    super().send(f"ISOP {nick}\r\n")

            if not sender_authorized:
                self.log(f"[AUTH] Rejected nick-prefixed command from {nick}: not authorized")
                return True  # Message handled (rejected)

            # Execute the command
            cmd_text = text[len(self.name) + 1:]  # strip "<own_nick> "
            if cmd_text.startswith("AI "):
                self.log(f"[EXEC] Nick-prefixed command from {nick}: {cmd_text}")
                # Send as a service command to the server
                reply_target = target if target.startswith("#") else nick
                self._last_message_sent = f"PRIVMSG {reply_target} :{cmd_text}\r\n"
                super().send(self._last_message_sent)
            return True  # Message handled

        # Message not handled, should be processed by AI model
        return False
    ...
    def process_command(self, cmd: str):
        ...
        if cmd.startswith("/"):
            ...
            if command == "/quit":
                ...
                if args:
                    super().send(f"WHOWAS {args}\r\n")
                else:
                    print("Usage: /whowas <nick>")
            elif command == "/mode":
                if args:
                    parts_split = args.split()
                    if len(parts_split) == 1:
                        super().send(f"MODE {parts_split[0]}\r\n")
                    elif len(parts_split) >= 2:
                        target = parts_split[0]
                        modestring = parts_split[1]
                        params = " ".join(parts_split[2:]) if len(parts_split) > 2 else ""
                        if params:
                            super().send(f"MODE {target} {modestring} {params}\r\n")
                        else:
                            super().send(f"MODE {target} {modestring}\r\n")
                else:
                    print("Usage: /mode <target> [modestring] [param...]")
            elif command == "/notice":
                parts_split = args.split(" ", 1)
                if len(parts_split) >= 2:
                    target = parts_split[0]
                    text = parts_split[1]
                    super().send(f"NOTICE {target} :{text}\r\n")
                else:
                    print("Usage: /notice <target> <text>")
            elif command == "/invite":
                parts_split = args.split(" ", 1)
                if len(parts_split) >= 2:
                    nick = parts_split[0]
                    channel = parts_split[1]
                    super().send(f"INVITE {nick} {channel}\r\n")
                elif len(parts_split) == 1:
                    nick = parts_split[0]
                    super().send(f"INVITE {nick} {self.current_channel}\r\n")
                else:
                    print("Usage: /invite <nick> [<channel>]")
            elif command == "/away":
                if args:
                    super().send(f"AWAY :{args}\r\n")
                else:
                    super().send("AWAY\r\n")
            elif command == "/oper":
                parts_split = args.split(" ", 1)
