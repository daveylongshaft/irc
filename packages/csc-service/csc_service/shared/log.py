...
class Log(Root):
    ...
    def ftp_announce(self, message: str):
        """Write a line to logs/ftp_announce.log for the #ftp IRC feed.

        Format: [HH:MM:SS] [self.name] message
        Uses Data._write_ftp_announce() when available (MRO), falls back to direct append.
        """
        ts = time.strftime("%H:%M:%S")
        line = f"[{ts}] [{self.name}] {message}"
        if hasattr(self, "_write_ftp_announce"):
            self._write_ftp_announce(line)
        else:
            try:
