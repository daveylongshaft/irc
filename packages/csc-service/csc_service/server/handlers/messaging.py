    ...
    def _handle_privmsg(self, msg, addr):
        ...
        try:
            ...
            if target.startswith("#"):
                ...
                # S2S: Enqueue channel message for federation replication
                if hasattr(self.server, 'event_queue'):
                    self.server.enqueue(prefix, "PRIVMSG", normalized_target, text)
        ...
        try:
            ...
            if target.startswith("#"):
                ...
                if channel and channel.has_member(nick):
                    ...
                    # S2S: Enqueue NOTICE for federation replication
                    if hasattr(self.server, 'event_queue'):
                        self.server.enqueue(prefix, "NOTICE", normalized_target, text)
            else:
                # Private NOTICE to a nick
                self._maybe_replay_pm_buffer(target, nick)
                ...
                if not self.server.send_to_nick(target, out):
                    ...
                    if hasattr(self.server, 'event_queue'):
                        ...
                        if hasattr(self.server, 's2s_network'):
                            remote_info = self.server.s2s_network.get_user_from_network(target)
                        if remote_info:
                            self.server.enqueue(prefix, "NOTICE", target, text)
                            self.server.chat_buffer.append(target, nick, "NOTICE", text)
                else:
                    self.server.chat_buffer.append(target, nick, "NOTICE", text)
                    # S2S: Enqueue NOTICE PM for federation replication
                    if hasattr(self.server, 'event_queue'):
                        self.server.enqueue(prefix, "NOTICE", target, text)
        except Exception as e:
    ...
    def _handle_service_via_chatline(self, raw_line, addr, nick, channel=None):
        ...
        for line in str(result).splitlines():
            ...
            if channel:
                ...
                if hasattr(self.server, 'event_queue'):
                    self.server.enqueue(prefix, "PRIVMSG", channel, prefixed)
            else:
