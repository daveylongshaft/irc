...
class OperMixin:
    ...
    def _handle_away(self, msg, addr):
        ...
        try:
            ...
            # S2S: Enqueue AWAY for federation replication
            if hasattr(self.server, 'event_queue'):
                prefix = f"{nick}!{nick}@{SERVER_NAME}"
                away_msg = self.server.clients[addr].get("away_message", "")
                self.server.enqueue(prefix, "AWAY", "*", away_msg)
    ...
    def _handle_kick(self, msg, addr):
        ...
        try:
            ...
            # S2S: Enqueue KICK for federation replication
            if hasattr(self.server, 'event_queue'):
                self.server.enqueue(prefix, "KICK", chan_name, f"{target_nick} {reason}")
    ...
    def _server_kill(self, target_nick, reason="Disconnected"):
        ...
        # S2S: Enqueue QUIT for federation replication
        if hasattr(self.server, 'event_queue'):
            self.server.enqueue(prefix, "QUIT", "*", reason)
    ...
    def _handle_wallops(self, msg, addr):
        ...
        # S2S: Enqueue WALLOPS for federation replication
        if hasattr(self.server, 'event_queue'):
            wallops_prefix = f"{nick}!{nick}@{SERVER_NAME}"
            self.server.enqueue(wallops_prefix, "WALLOPS", "*", text)
