...
class ChannelMixin:
    ...
    def _handle_join(self, msg, addr):
        ...
        try:
            ...
            for chan_name in channels:
                ...
                # S2S: Enqueue JOIN for federation replication
                if hasattr(self.server, 'event_queue'):
                    self.server.enqueue(prefix, "JOIN", chan_name)
    ...
    def _handle_part(self, msg, addr):
        ...
        try:
            ...
            for chan_name in chan_names:
                ...
                # S2S: Enqueue PART for federation replication
                if hasattr(self.server, 'event_queue'):
                    self.server.enqueue(prefix, "PART", chan_name, reason)
        except (IndexError, ValueError) as e:
    ...
    def _handle_topic(self, msg, addr):
        ...
        try:
            ...
            if len(msg.params) < 2:
                ...
                # S2S: Enqueue TOPIC for federation replication
                if hasattr(self.server, 'event_queue'):
                    self.server.enqueue(prefix, "TOPIC", chan_name, new_topic)
    ...
    def _handle_invite(self, msg, addr):
        ...
        try:
            ...
            # S2S: Enqueue INVITE for federation replication
            if hasattr(self.server, 'event_queue'):
                self.server.enqueue(prefix, "INVITE", target_nick, chan_name)
        except (IndexError, ValueError) as e:
