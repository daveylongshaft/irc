...
def is_encrypted(data: bytes) -> bool:
    ...
    Detection logic:
        ...
        2. Try to decode first 20 bytes as UTF-8
        3. If first char is ':' or uppercase → plaintext IRC → return False
        4. If first word matches a known IRC command → return False
    ...
    if len(data) < 4:
        return False
    try:
        start = data[:20].decode("utf-8")
        if start[0] == ":" or start[0].isupper():
            return False
        if start.split()[0] in ("NICK", "USER", "PING", "PONG", "QUIT", "PASS",
