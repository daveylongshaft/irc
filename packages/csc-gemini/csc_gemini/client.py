...
class GeminiClient(AIClient):
    ...
    def __init__(self, config_path=None, input_file=None, output_file=None):
        ...
        model = self._perform.get("ai", "model", "gemini-2.0-flash")
        self._api = GeminiAPI(api_key, model=model)
    ...
    def _resolve_api_key(self):
        ...
        if key:
            return key
        raise ValueError(
            "[GeminiClient] CRITICAL: No API key found. "
            "Set GOOGLE_API_KEY or GEMINI_API_KEY env var or store gemini_api_key in the data store."
        )
