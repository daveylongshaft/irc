...
class FtpVirtualFs(AbstractedFS):
    ...
    def mkdir(self, path):
        ...
        log.debug(f"Virtual FS ignoring mkdir request for {path}")
    ...
    def chmod(self, path, mode):
        """Chmod is not supported in virtual FS -- no-op."""
        log.debug(f"Virtual FS ignoring chmod request for {path} to {mode}")
